...
fn generate<F: RichField + Extendable<D>, const D: usize>(
    ...
    let permutation_input_vals = (0..KECCAK_WIDTH_U32S)
        ...
        .map(|col| {
            ...
            for (_, val) in permutation_trace[col]
                .values
                .iter()
                .enumerate()
                .filter(|&(i, _)| {
                    permutation_trace[permutation_layout::REG_INPUT_FILTER].values[i] == F::ONE
                })
            {
                vals.insert(*val);
            }
    ...
    let permutation_output_vals = (0..KECCAK_WIDTH_U32S)
        ...
        .map(|col| {
            ...
            for (_, val) in permutation_trace[col]
                ...
                .enumerate()
                .filter(|&(i, _)| {
                    permutation_trace[permutation_layout::REG_OUTPUT_FILTER].values[i] == F::ONE
                })
            {
                vals.insert(*val);
    ...
    for i in 0..KECCAK_WIDTH_U32S {
        ...
        for row in 0..sponge_trace[col].values.len() {
            ...
            if filter {
                ...
                assert!(
                    permutation_output_vals[i].contains(&sponge_trace[col].values[row]),
                    "permutation output value not found: value: {}, sponge_col: {}. sponge_row: {}",
                    sponge_trace[col].values[row],
                    col,
                    row
                );
