...
use starky2::starky2lib::keccak256_sponge::{
    layout as sponge_layout, layout::KECCAK_RATE_U32S, Keccak256SpongeStark,
};
...
// 	[(); 3 + 2 * KECCAK_RATE_U32S]:,
{
    ...
    fn get_ctl_descriptor(&self) -> CtlDescriptor {
        ...
        let instances =
            sponge_layout::xor_ctl_cols_a(SPONGE_TID)
                .zip(XorLayout::<F, KECCAK_RATE_U32S>::ctl_cols_a(XOR_TID));
        let instances =
        ...
        let instances = instances.chain(
            sponge_layout::xor_ctl_cols_output(SPONGE_TID)
                .zip(XorLayout::<F, KECCAK_RATE_U32S>::ctl_cols_output(XOR_TID)),
        );
        ...
        let instances =
            instances
                .chain(sponge_layout::keccak_ctl_col_input(SPONGE_TID).zip(
                    (0..KECCAK_RATE_U32S).map(|i| {
                        CtlColumn::new(
                            PERMUTATION_TID,
    ...
    fn generate(
        ...
        for data in datas.iter() {
            let (_id, _hash, state_trace, xored_rate_trace) =
                sponge_generator.gen_hash_with_trace(data);

            let xor_pairs = state_trace
                .iter()
                .map(|x| *array_ref![x, 0, KECCAK_RATE_U32S])
                .take(xored_rate_trace.len())
                .zip(xored_rate_trace.iter().copied());
            xor_pairs_seen.extend(xor_pairs);

            let xored_extended = xored_rate_trace
                .into_iter()
                .chain(std::iter::repeat([0u32; KECCAK_RATE_U32S]));
            let keccak_inputs_u64 =
                state_trace
                    .into_iter()
                    .zip(xored_extended)
                    .map(|(mut state, xored_rate)| {
