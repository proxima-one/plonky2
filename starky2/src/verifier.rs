...
use crate::ro_memory::{RoMemoryCheckVars, RoMemoryDescriptor};
    ...
    [(); C::Hasher::HASH_SIZE]:,
{
    ...
    let ro_memory_vars = if let Some(descriptors) = stark.ro_memory_descriptors() {
        ...
        Some(RoMemoryCheckVars {
            ...
            next_pps: ro_memory_pps_next.as_ref().unwrap().clone(),
            challenges: challenges.ro_memory_challenges.clone().unwrap(),
            addr_cols,
            value_cols,
            addr_sorted_cols,
            value_sorted_cols,
