...
impl RoMemoryDescriptor {
    pub fn new(
        addr_col: usize,
        value_col: usize,
        addr_sorted_col: usize,
        value_sorted_col: usize,
    ) -> Self {
        Self {
            addr_col,
            value_col,
...
pub(crate) fn get_ro_memory_challenge<F: RichField, H: Hasher<F>>(
...
pub(crate) fn get_n_ro_memory_challenges<F: RichField, H: Hasher<F>>(
    challenger: &mut Challenger<F, H>,
    n: usize,
) -> Vec<RoMemoryChallenge<F>> {
    (0..n)
        .map(|_| get_ro_memory_challenge(challenger))
        .collect()
}
...
pub(crate) fn get_ro_memory_data<
...
>(
    ...
    let ro_memory_descriptors = stark
        .ro_memory_descriptors()
        .expect("ro_memory_descriptors() should be Some");
    ...
    for descriptor in ro_memory_descriptors {
        ...
        let new_products = new_challenges
            ...
            .map(|challenge| {
                ...
                let product_poly = izip!(
                    addr_values.values.iter(),
                    value_values.values.iter(),
                    addr_sorted_values.values.iter(),
                    value_sorted_values.values.iter()
                )
    ...
    S: Stark<F, D>,
{
    ...
    let instances = izip!(
        ...
        addr_cols.iter(),
        value_cols.iter(),
        addr_sorted_cols.iter(),
        value_sorted_cols.iter()
    );
    let pps = izip!(local_pps.iter(), next_pps.iter());
    for (
        (
            &RoMemoryChallenge { alpha, zeta },
            &addr_col,
            &value_col,
            &addr_sorted_col,
            &value_sorted_col,
    ...
    ) in instances.zip(pps)
    {
        ...
        let lhs = -(curr_row[addr_sorted_col]
            + curr_row[value_sorted_col] * FE::from_basefield(alpha))
            + FE::from_basefield(zeta);
        let rhs = -(curr_row[addr_col] + curr_row[value_col] * FE::from_basefield(alpha))
            + FE::from_basefield(zeta);
        yield_constr.constraint_first_row(lhs * local_pp - rhs);
        ...
        let lhs = -(next_row[addr_sorted_col]
            + next_row[value_sorted_col] * FE::from_basefield(alpha))
            + FE::from_basefield(zeta);
        let rhs = -(next_row[addr_col] + next_row[value_col] * FE::from_basefield(alpha))
            + FE::from_basefield(zeta);
        yield_constr.constraint_transition(lhs * next_pp - rhs * local_pp);
    }
}
