    ...
    ($channels:expr) => {
            ...
            for StackStark<F, D, $channels>
        {
                ...
                P: PackedField<Scalar = FE>,
            {
				...
				yield_constr.constraint_transition(curr_row.is_pop * (next_row.sp - curr_row.sp + P::ONES));
				...
				yield_constr.constraint((P::ONES - curr_row.is_pop) * (curr_row.addr - curr_row.sp));
				yield_constr.constraint_transition((P::ONES - curr_row.is_pop) * (next_row.sp - curr_row.sp - P::ONES));
				yield_constr.constraint((P::ONES - curr_row.is_pop) * (P::ONES - curr_row.is_write));
...
mod tests {
	...
	fn test_random_trace() -> Result<()> {
		...
		let mut height = 0;
		for i in 0..500 {
			let is_pop = if height == 0 { false } else { rng.gen_range(0..2) == 1 };
