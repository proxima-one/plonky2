...
pub trait RecursiveConstraintConsumerFiltered<F: RichField + Extendable<D>, const D: usize> {
	fn constraint_filtered(&mut self, builder: &mut CircuitBuilder<F, D>, c: ExtensionTarget<D>, filter: ExtensionTarget<D>);
	fn constraint_transition_filtered(&mut self, builder: &mut CircuitBuilder<F, D>, c: ExtensionTarget<D>, filter: ExtensionTarget<D>);
}

impl<F: RichField + Extendable<D>, const D: usize> RecursiveConstraintConsumerFiltered<F, D> for RecursiveConstraintConsumer<F, D> {
	fn constraint_filtered(&mut self, builder: &mut CircuitBuilder<F, D>, c: ExtensionTarget<D>, filter: ExtensionTarget<D>) {
...
impl<F: RichField + Extendable<D>, const D: usize> RecursiveStarky2ConstraintConsumer<F, D> for RecursiveConstraintConsumer<F, D> {
	fn inv_check(&mut self, builder: &mut CircuitBuilder<F, D>, value: ExtensionTarget<D>, inv: ExtensionTarget<D>, binary_flag: ExtensionTarget<D>) {
	...
	fn mutually_exclusive_binary_check(&mut self, builder: &mut CircuitBuilder<F, D>, values: &[ExtensionTarget<D>]) {
		...
		let sum = values.iter().fold(builder.zero_extension(), |acc, val| builder.add_extension(acc, *val));
