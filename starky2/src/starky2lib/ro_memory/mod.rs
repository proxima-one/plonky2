...
impl<F: RichField + Extendable<D>, const D: usize, const NUM_CHANNELS: usize> RoMemoryStark<F, D, NUM_CHANNELS> {
	pub fn new() -> Self {
		Self {
...
macro_rules! impl_ro_memory_stark_for_n_channels {
	($channels:expr) => {
		impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for RoMemoryStark<F, D, $channels> {
                ...
                P: PackedField<Scalar = FE>,
            {
				...
				yield_constr.constraint_transition(
					(next_row.addr_sorted - curr_row.addr_sorted) * (next_row.addr_sorted - curr_row.addr_sorted - P::ONES)
				);
			}
