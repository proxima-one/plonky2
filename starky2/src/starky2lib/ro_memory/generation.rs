	...
	[(); RO_MEMORY_NUM_COLS_BASE + NUM_CHANNELS]:
{
	...
	pub fn gen_access_trace<I: Iterator<Item = (F, F)>>(&mut self, accesses: &mut I, channels: &[usize]) {
	...
	fn gen_sorted_cols(&mut self) {
		let sorted_accesses = self.trace.iter().enumerate().map(|(i, row)| {
			let row: &RoMemoryRow<F, NUM_CHANNELS> = row.borrow();
			(row.addr, i)
		}).sorted_by_cached_key(|(addr, _)| addr.to_canonical_u64()).collect_vec();
	...
	pub fn into_polynomial_values_with_degree(mut self, degree_bits: usize) -> Vec<PolynomialValues<F>> {
