...
where
    [(); SLICE_CHECK_NUM_COLS_BASE + NUM_CHANNELS]:,
{
    pub fn new(
        left_mem: &'a [F],
        right_mem: &'a [F],
    ) -> SliceCheckRowGenerator<'a, F, NUM_CHANNELS> {
    ...
    pub fn gen_slice_check(
        ...
        while self.remaining_len > 0 {
            ...
            self.right_addr += 1;
            assert_eq!(
                self.left_mem[self.left_addr],
                self.right_mem[self.right_addr]
            );
            row = self.gen_row();
            self.trace.push(row);
        }
