...
pub fn get_challenges_and_fri_instance<'a, 'b, C: GenericConfig<D>, const D: usize>(
    ...
    [(); C::Hasher::HASH_SIZE]:,
{
    ...
    let pis_hash = proof_buf.read_pis_hash()?;
    ...
    let vanishing_polys_zeta = eval_vanishing_poly::<C::F, C, D>(
        ...
        partial_products,
        s_sigmas,
        plonk_betas.as_slice(),
        plonk_gammas.as_slice(),
        plonk_alphas.as_slice(),
        gates.as_slice(),
        &selectors_info,
        k_is.as_slice(),
        degree_bits,
    ...
    let zeta_pow_deg = plonk_zeta.exp_power_of_2(degree_bits);
...
mod tests {
    ...
    fn test_buffer_verifier() -> Result<()> {
        ...
        get_challenges_and_fri_instance(&mut proof_buf, &mut circuit_buf)?;
        let mut proof_buf = ProofBuf::<C, &[u8], D>::new(proof_bytes.as_slice())?;

        verify_constraints(&mut proof_buf, &mut circuit_buf)?;
        let num_rounds = circuit_buf.read_fri_num_query_rounds()?;
        for round in 0..num_rounds {
            verify_fri_round(&mut proof_buf, &mut circuit_buf, round)?;
        }
