...
use crate::fri::structure::{FriInstanceInfo, FriOracleInfo, FriPolynomialInfo, FriBatchInfo};
...
impl<'a, C: GenericConfig<D>, const D: usize> CircuitBuf<'a, C, D> {
    ...
    pub fn read_selectors_info(&self) -> SelectorsInfo {
        let len = (self.offsets.selector_groups_offset - self.offsets.selector_indices_offset) / std::mem::size_of::<u64>();
        let selector_indices = read_usize_vec(self.buf, self.offsets.selector_indices_offset, len);

        let len = (self.offsets.degree_bits_offset - self.offsets.selector_groups_offset) / (std::mem::size_of::<u64>() * 2);
    ...
    pub fn read_k_is(&self) -> Vec<C::F> {
        let len = (self.offsets.num_partial_products_offset - self.offsets.k_is_offset) / std::mem::size_of::<u64>();
    ...
    pub fn read_sigmas_cap(&self) -> MerkleCap<C::F, C::Hasher> {
        let len = (self.offsets.sigmas_cap_offset - self.offsets.quotient_degree_factor_offset) / std::mem::size_of::<u64>();
    ...
    pub fn read_fri_instance(&self) -> FriInstanceInfo<C::F, D> {
        let len = (self.offsets.fri_instance_batches_offset - self.offsets.fri_instance_oracles_offset) / std::mem::size_of::<u8>();
        let blinding_vec = read_bool_vec(self.buf, self.offsets.fri_instance_oracles_offset, len);
        let oracles = blinding_vec.into_iter().map(|blinding| FriOracleInfo { blinding }).collect();
        
        let num_batches = LittleEndian::read_u64(&self.buf[self.offsets.fri_instance_batches_offset..]) as usize;
        ...
        for _ in 0..num_batches {
            ...
            for _ in 0..num_polys {
                ...
                polynomials.push(FriPolynomialInfo { oracle_index, polynomial_index });
            }
...
pub fn read_merkle_cap<C: GenericConfig<D>, const D: usize>(buf: &[u8], offset: usize, width: usize) -> MerkleCap<C::F, C::Hasher> {
...
pub fn read_field_ext_vec<C: GenericConfig<D>, const D: usize>(buf: &[u8], mut offset: usize, len: usize) -> Vec<C::FE> {
...
pub fn read_field_vec<C: GenericConfig<D>, const D: usize>(buf: &[u8], offset: usize, len: usize) -> Vec<C::F> {
    let mut res = Vec::with_capacity(len);
    let buf = &buf[offset..];
    for i in 0..len {
