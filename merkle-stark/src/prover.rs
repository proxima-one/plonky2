...
use crate::proof::{Merkle5STARKProof, StarkOpeningSet, StarkProof, StarkProofWithPublicInputs};
    ...
    [(); MerkleTree5STARK::<F, D>::COLUMNS]:,
{
    ...
    let tree_proof = prove_single_table(
        &all_stark.tree_stark,
        config,
        &trace_poly_values[Table::Tree as usize],
        &trace_commitments[Table::Tree as usize],
        &ctl_data_per_table[Table::Tree as usize],
        public_inputs[Table::Tree as usize]
            .clone()
            .try_into()
            .unwrap(),
        &mut challenger,
    ...
    let hash_proof = prove_single_table(
        &all_stark.hash_stark,
        config,
        &trace_poly_values[Table::Hash as usize],
        &trace_commitments[Table::Hash as usize],
        &ctl_data_per_table[Table::Hash as usize],
        public_inputs[Table::Hash as usize]
            .clone()
            .try_into()
            .unwrap(),
        &mut challenger,
    ...
    let stark_proofs = vec![tree_proof, hash_proof];
    debug_assert_eq!(stark_proofs.len(), num_starks);

    Ok(Merkle5STARKProof { stark_proofs })
}
