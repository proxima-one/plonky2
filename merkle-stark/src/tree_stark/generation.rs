	...
	pub fn gen(&mut self) -> [u32; 8] {
		...
		for level in 0..(TREE_DEPTH - 1) {
			for _ in 0..level_width(level + 1) {
				...
				for word in 0..WORDS_PER_HASH {
					curr_row[hash_input_0_word(word)] = curr_row[val_i_word(0, word)] + curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
					curr_row[hash_input_1_word(word)] = curr_row[val_i_word(1, word)] + curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
				}
				...
				for word in 0..WORDS_PER_HASH {
					...
					let left_field = left_with_idx - curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
					let right_field = right_with_idx - curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);

					left[word] = left_field.to_canonical_u64().try_into().expect("expected hash word to fit into u32");
					right[word] = right_field.to_canonical_u64().try_into().expect("expected hash word to fit into u32");
				}

				let output_hash = compress(left, right);
				for word in 0..WORDS_PER_HASH {
					curr_row[hash_output_word(word)] = F::from_canonical_u32(output_hash[word]) + curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
				}
				...
				if level_done && level < TREE_DEPTH - 1 {
					...
					for i in 0..(next_level_width - 1) {
						...
						for word in 0..WORDS_PER_HASH {
							next_row[val_i_word(i, word)] = curr_row[val_i_word(i + shift_amount, word)];
						}
					}

					// append the hash from OUTPUT_COL
					for word in 0..WORDS_PER_HASH {
						next_row[val_i_word(next_level_width - 1, word)] = curr_row[hash_output_word(word)] - curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
					}
					...
					for word in 0..WORDS_PER_HASH {
						next_row[val_i_word(TREE_WIDTH - 1, word)] = curr_row[hash_output_word(word)] - curr_row[HASH_IDX] * F::from_canonical_u64(1 << 32);
					}
				}
		...
		for word in 0..WORDS_PER_HASH {
			root[word] = last_row[val_i_word(0, word)].to_canonical_u64().try_into().expect("expected hash word to fit in u32");
		}
