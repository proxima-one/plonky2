...
impl<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> Merkle5STARKProof<F, C, D> {
    ...
    pub(crate) fn get_challenges(
        ...
        config: &StarkConfig,
    ) -> Merkle5STARKProofChallenges<F, D> {
...
impl<const D: usize> Merkle5STARKProofTarget<D> {
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        Merkle5STARKProofChallengesTarget {
            stark_challenges: izip!(
                &self.stark_proofs,
                all_stark.nums_permutation_zs(config),
                all_stark.permutation_batch_sizes()
